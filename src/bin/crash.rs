...
fn main() {
    ...
    if argv.first().map(String::as_str) != Some("crc-speed") {
        eprintln!("crc-speed: optional argument: --out DIR");
        std::process::exit(2);
    }
    ...
    let mut csv = Csv::open(&out.join("crc-speed.csv"), "size,gb_per_s,us_per_call,checksum").expect("csv");
    ...
    for size in [1024usize, 65536] {
        ...
        csv.row(&[size.to_string(), format!("{gb_per_s:.2}"), format!("{per_call_us:.2}"), checksum.to_string()]).expect("row");
