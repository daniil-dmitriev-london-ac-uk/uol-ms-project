...
impl BenchmarkHeap {
    fn open(layout: &str, io: &str, dir: &Path, config: HeapConfig) -> BenchmarkHeap {
        ...
        match (layout, io) {
            ("append", "sync") => { BenchmarkHeap::SyncAppend(Heap::open(dir, SyncIo::new(), config).expect("open")) }
            ("bucket", "sync") => { BenchmarkHeap::SyncBucket(Heap::open(dir, SyncIo::new(), config).expect("open")) }
            ("append", "uring") => { BenchmarkHeap::UringAppend(Heap::open(dir, uring(), config).expect("open")) }
            ("bucket", "uring") => { BenchmarkHeap::UringBucket(Heap::open(dir, uring(), config).expect("open")) }
...
fn populate(layout: &str, dir: &Path, size: u64, count: u64, buckets: u64) -> Vec<u64> {
    ...
    let mut heap = BenchmarkHeap::open(layout, "sync", dir, config_for(size * count, SyncPolicy::None));
    let mut payload = vec![0u8; size as usize];

    SplitMix64::new(42).fill(&mut payload);

    let ids: Vec<u64> = (0..count).map(|key| heap.insert_into(key % buckets, &payload)).collect();
...
fn governor() -> String {
    std::fs::read_to_string("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor").unwrap_or_default().trim().to_string()
}
...
fn run_repetition(op_bytes: u64, mut operation: impl FnMut(u64) -> u64) -> Vec<u64> {
    ...
    loop {
        ...
        if (iteration >= minimum_iterations && elapsed >= target_seconds) || iteration >= maximum_iterations || elapsed >= maximum_seconds { break; }
    }
...
fn summary_row( csv: &mut Csv, tag: &[&str], results: &[RepetitionResult], records: u64, op_bytes: u64) {
    ...
    let iterations: u64 = results.iter().map(|repetition| repetition.stats.iterations).sum();
    ...
    let device_reads_per_operation = median_float(&|repetition| per_operation(repetition.io_counters.reads, repetition));
    let device_writes_per_operation = median_float(&|repetition| per_operation(repetition.io_counters.writes, repetition));
    let read_bytes_per_operation = median_float(&|repetition| per_operation(repetition.io_counters.read_bytes, repetition));
    let write_bytes_per_operation = median_float(&|repetition| per_operation(repetition.io_counters.write_bytes, repetition));
...
fn parse(argv: &[String]) -> Args {
    ...
    while argument_index < argv.len() {
        match ( argv.get(argument_index).map(String::as_str), argv.get(argument_index + 1)) {
            ...
            (Some(flag), _) => panic!("no argument {flag}"),
            (None, _) => break,
        }
...
fn main() {
    ...
    if argv.first().map(String::as_str) != Some("matrix") {
        eprintln!("matrix: arguments: --dir --out --filter --reps");
        std::process::exit(2);
    }
