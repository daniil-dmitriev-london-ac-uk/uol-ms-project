...
pub(super) fn cost_sync(args: &Args, csv: &mut Csv) {
    for layout in ["append", "bucket"] {
        for (policy, policy_name) in [(SyncPolicy::None, "none"), (SyncPolicy::EveryN(32), "every32"), (SyncPolicy::EveryN(8), "every8"),(SyncPolicy::EveryN(1), "every1")]
        {
            ...
            let write_amplification = counters.write_bytes as f64 / (stats.iterations * 1024) as f64;
            ...
            println!("cost-sync {layout} {policy_name}: ops/s={:.0} p99={:.0}us WA={:.2}", operations_per_second, stats.p99_ns as f64 / 1000.0, write_amplification);
        ...
        for (size, size_name) in [(1024usize, "1K"), (65536, "64K")] {
            ...
            for _round in 0..ROUNDS {
                for (integrity_index, integrity) in [true, false].into_iter().enumerate() {
                    ...
                    let _ = std::fs::remove_dir_all(&dir);
                    let (mut heap, _) = LayoutHeap::open(layout, &dir, heap_config(SyncPolicy::None, integrity));
                    let payload = make_payload(2, size);
                    let started = Instant::now();
                    let ids: Vec<u64> = (0..record_count).map(|key| heap.insert_into(key % BUCKET_COUNT, &payload)).collect();

                    heap.flush();

                    write_rates[integrity_index].push(record_count as f64 / started.elapsed().as_secs_f64());
                    ...
                    for _ in 0..reads {
                        heap.read(ids[rng.below(record_count) as usize], &mut out).unwrap();
                    }

                    read_rates[integrity_index].push(reads as f64 / started.elapsed().as_secs_f64());
            ...
            for (integrity_index, integrity) in [true, false].into_iter().enumerate() {
                let (mut sorted_write_rates, mut sorted_read_rates) = (write_rates[integrity_index].clone(), read_rates[integrity_index].clone());

                let (median_write_rate, median_read_rate) = (median_f64(&mut sorted_write_rates), median_f64(&mut sorted_read_rates));
                ...
                println!(
                    "integrity of cost {layout} {size_name} crc={integrity}: insert/s={median_write_rate:.0} ({:.0}..{:.0}) read/s={median_read_rate:.0} ({:.0}..{:.0})",
...
pub(super) fn sync_shape(args: &Args, csv: &mut Csv) {
    for regions in [1u64, 2, 4, 8, 16] {
        ...
        let _ = std::fs::remove_dir_all(&dir);
        let (mut heap, _) = LayoutHeap::open("bucket", &dir, heap_config(SyncPolicy::EveryN(8), true));
        let payload = make_payload(1, 1024);
...
pub(super) fn crc_speed(args: &Args, csv: &mut Csv) {
    for size in [1024usize, 65536] {
        ...
        println!("crc speed {size} bytes: {gigabytes_per_second:.2} gb/s, {microseconds_per_call:.2} us per call");
    }
